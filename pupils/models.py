...
class Pupil(models.Model):
    ...
    user_info = models.ForeignKey(User, on_delete=models.CASCADE)
    parent_info = models.ForeignKey(Parent, on_delete=models.CASCADE)
    ...
    b_mon = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Monday Breakfast")
    b_tues = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Tuesday Breakfast")
    b_wed = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Wednesday Breakfast")
    b_thurs = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Thursday Breakfast")
    b_fri = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Friday Breakfast")
    ...
    s_wed = models.BooleanField(
        null=False, blank=False, default=False, verbose_name="Wednesday Supper")
...
class DateRequest(models.Model):
    ...
    REQUEST_TYPES = (
        ('0', 'Request an additional breakfast'),
        ('1', 'Request an additional supper'),
        ('2', 'Cancel existing breakfast'),
        ('3', 'Cancel existing supper'),
        )
    APPROVAL_CHOICES = (
        ('0', 'Awaiting Approval'),
        ('1', 'Approved'),
    ...
    approval_status = models.CharField(
        max_length=1, choices=APPROVAL_CHOICES, default='0')
    why_declined = models.CharField(
        max_length=1, choices=DECLINED_REASONING, default='0'
    )
    type_of_request = models.CharField(
        max_length=1, choices=REQUEST_TYPES)
    approved = models.BooleanField(null=False, blank=False, default=False)
    request_date = models.DateTimeField(
        auto_now_add=True,
        verbose_name="Date request was made")
